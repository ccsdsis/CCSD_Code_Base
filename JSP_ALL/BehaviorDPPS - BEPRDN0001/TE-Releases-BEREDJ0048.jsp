...
// TE-Releases-BEREDJ0048.jsp
// SIS-6096 2018-06-07 (FM) Add "date override" functionality to bypass TE Review Date logic that requires the date be >= current date. 
// SIS-5647 2017-12-19 (FM) New - Custom Report - Behavior>Behavior DPPS>Trial Enrollment Releases
//                          Input criteria: TE ReviewDate (@reviewDateIn) populated from Behavior DPPS records where DPPS Close Date is null
//
...
// baseURL
	...
	String srsReportName = "TE-Releases-BEREDR0048";
	String jspPageName   = "TE-Releases-BEREDJ0048";
...
%>
	...
	<TABLE cellpadding="0" cellspacing="0" width="640">
		<TR>
		<TD class="wizardHeader" width="100%" style="height: 21px;">&nbsp; &nbsp; &nbsp;Trial Enrollment Releases</TD>
		</TR>
		
		<TR>
			<TD class="wizardInstruction" style="padding-top: 10px; padding-right: 30px; padding-left: 30px; height=33px;" valign="top">
			This Behavior DPPS report generates a list of Trial Enrollment Releases.<BR>
			Selected records do not have a Close Date and have a TE Review Date greater than or equal to today.</br><br>If TE Review Date Override is selected, all TE Release records that do not have a Close Date are reported. The TE Review Date can be earlier than today.<br><br>This report is generated in CSV or Excel formats.<br></br>
			</TD>
		</TR>
 ...
 <%
			...
			int newRecords1 = 0;						
			SQL1 =  " DECLARE @TEReviewDate      int = (select attributeid from campusattribute with (nolock) where object='Behavior DPPS' and element='TEReviewDate') " +
					...
					" WHERE cs.attributeid = @TEReviewDate " +
					"  AND cs2.date      IS NULL " +
					" ORDER by reviewDateRefmt asc " ;

			result1 = db2.executeQuery(SQL1);
			out.println("<label><br><strong>&nbsp;&nbsp;&nbsp;Select TE Review Date:</strong></label><br/><select name='reviewDateList' id='reviewDateList'><option value='0'>SELECT ALL</option>");
			while(result1.next())
...
%>
            ...
            <TD class="detailFormColumn" style="padding-top: 11px;"><STRONG><label style="font-family:arial, helvetica, sans-serif"></br>TE Review Date Override:</label></STRONG>
			<BR>
