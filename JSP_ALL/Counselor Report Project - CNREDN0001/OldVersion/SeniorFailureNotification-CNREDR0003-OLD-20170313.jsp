...
// baseURL
	...
	String srsReportName = "SeniorFailureNotification-CNREDR0003";
	String jspPageName = "SeniorFailureNotification-CNREDR0003";
...
<%
	...
	try
	{
		...
		String url = path+dir+srsReportName+"&schoolID="+schoolID+"&staffPersonID="+personIDValue+"&endYear="+endYear+"&grade="+GradeValue+"&termID="+TermValue+"&TestDate="+TestDateValue+"&EndTestDate="+EndTestDateValue;
  ...
  <script LANGUAGE="JavaScript">
      function validate() 
	  {
		...
		if (select.value == "NULL") {
		  alert("At least one counselor must be selected.");
		  return false;
		}
		
		select = document.getElementById('17');
		if (select.value == "null") {
		  alert("A grade must be selected.");
		  return false;
		}
...
%>
	...
	<TABLE cellpadding="0" cellspacing="0" width="640">
		<TR>
		<TD class="wizardHeader" width="100%" style="height: 21px;">&nbsp;Senior Failure Notification</TD>
		</TR>
		
		<TR>
		<TD class="wizardInstruction" style="padding-top: 10px; padding-right: 30px; padding-left: 30px; height=33px;" valign="top">
		The Senior Failure Notification report produces a letter for each student who is currently failing one or more courses for the selected counselor(s).<br/><BR/><BR/>
		</TD>
		</TR>
...
%>		
		...
		<td><small><br><br><label style="font-size:small;font-family:arial, helvetica, sans-serif">Please select the report rendering format</label>&nbsp;<strong></strong>
			<span style="font-size:small;font-family:arial, helvetica, sans-serif"><br></span>
			        <input type="radio" name="render" value="pdfPDF" checked><span style="font-size:small;font-family:arial, helvetica, sans-serif">PDF<br></span>
		...
		<TR>
		<TD><br><font color=red size=3> * </font> = Required field<br>
