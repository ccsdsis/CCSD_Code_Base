...
// baseURL
	...
	String EndTestDateValue = request.getParameter("EndTestDate");	
	...
	String srsReportName = "SeniorFailureNotificationByStudent-CNREDR0003";
	String jspPageName = "SeniorFailureNotificationByStudent-CNREDR0003";
...
<%
	...
	try
	{
		...
		String url = path+dir+srsReportName+"&schoolID="+schoolID+"&endYear="+endYear+"&termID="+TermValue+"&TestDate="+TestDateValue+"&personID="+AllPersonIDs+"&EndTestDate="+EndTestDateValue;
...
<link rel="stylesheet" href="//code.jquery.com/ui/1.10.4/themes/smoothness/jquery-ui.css">
  ...
  <script LANGUAGE="JavaScript">
      function validate() 
	  {
        var select = document.getElementById('16');
		...
		select = document.getElementById('18');
		if (select.value == "null") {
		  alert("A term must be selected.");
		  return false;
		}
		...
		if(date1.value != "" && date2.value != "")
		{
			...
			if(d2 <= d1)
			{
				alert("End date is on or before start date.");
				return false;
			}
		}
...
%>
	...
	<FORM name = 'rsPass' action="<%= pagePath %><%= jspPageName %>.jsp?callrspassthru=true" method="post" onsubmit="return validate();"> <!--(this version works for most districts-->
	
	<TABLE cellpadding="0" cellspacing="0" width="640">
		<TR>
		<TD class="wizardHeader" width="100%" style="height: 21px;">&nbsp;Senior Failure Notification by Student</TD>
		</TR>
		
		<TR>
		<TD class="wizardInstruction" style="padding-top: 10px; padding-right: 30px; padding-left: 30px; height=33px;" valign="top">
		The Senior Failure Notification report produces a letter for each student selected, who is currently failing one or more courses.<br><BR><BR>
		</TD>
		</TR>
 ...
 <%
			...
			out.println("<label> <strong>Student</strong></label><font color=red size=3> * </font><br/><label>To Select Multiple Students, hold the CTRL button and select students</label><br/><select name=personIDList multiple id=16 style='width:250px; height:300px;' class='required'><option value='NULL' selected='true' style='color: black;'>Select</option>");
			while(result2.next())
			{ personID2=result2.getString(2); 
			StudentName = result2.getString(1);
			out.println("<option value='"+personID2+"' style='color: black;'>"+StudentName+"</option>");}
			out.println("</select>");
...
<%
			...
			out.println("<br/><label>Select Term:</label><br><select name=TermValueList id=18 ><option value='null' selected='false'> </option>");
			while(result4.next())
			{ TermValue = result4.getString(1);
                        TermName = result4.getString(2);
			out.println("<option value='"+TermValue+"'>"+TermName+"</option>");}
			out.println("</select>");
			out.println("<font color=red size=3> * </font>");
...
%>		
                ...
                <TR>
                    <td ></br><br>Select EOC Exam Window Start Date:
                    <input type="text" name="TestDate" id="datepicker"><font color=red size=3> * </font> 
                    </td>
                </TR>
				<TR>
                    <td ></br><br>Select EOC Exam Window End Date:
                    <input type="text" name="EndTestDate" id="datepicker2"><font color=red size=3> * </font> 
                    </td>
                </TR>
		<tr>
		<td><small><br><br><label style="font-size:small;font-family:arial, helvetica, sans-serif">Please select the report rendering format</label>&nbsp;<strong></strong>
			<span style="font-size:small;font-family:arial, helvetica, sans-serif"><br></span><input type="radio" name="render" value="pdfPDF" checked><span style="font-size:small;font-family:arial, helvetica, sans-serif">PDF<br></span>
		...
		<TR>
		<TD><br><font color=red size=3> * </font> = Required field
				<br>
